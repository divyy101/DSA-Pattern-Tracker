          ...
          <>
            <div className="flex items-start justify-between gap-3">
              <div className="flex items-start gap-2.5 flex-1 min-w-0">
                <span className="shrink-0 w-6 h-6 rounded-lg flex items-center justify-center text-[0.7rem] font-black bg-violet-500/10 text-violet-400 border border-violet-500/20 mt-0.5">
                  {orderNumber}
                </span>
    ...
    <div className="flex flex-col gap-8">
      ...
      <div>
        <div className="text-sm font-bold text-slate-200 mb-4 flex items-center gap-2">
          <span>🔥</span> Unsolved Problems
          <span className="px-2 py-0.5 text-xs font-semibold bg-rose-500/10 text-rose-400 border border-rose-500/20 rounded-full">{unsolvedProblems.length}</span>
        </div>
        {unsolvedProblems.length === 0 ? (
          <div className="text-center py-8 px-4 bg-[#0f1428]/30 border border-dashed border-slate-800 rounded-2xl animate-fade-slide-up">
            <span className="text-2xl block mb-2">🎉</span>
            <p className="text-xs text-slate-500 font-medium">All caught up! No unsolved problems.</p>
          </div>
        ) : (
      ...
      <div>
        <div className="text-sm font-bold text-slate-200 mb-4 flex items-center gap-2">
          ...
          <span className="px-2 py-0.5 text-xs font-semibold bg-emerald-500/10 text-emerald-400 border border-emerald-500/20 rounded-full">{solvedProblems.length}</span>
        </div>
        {solvedProblems.length === 0 ? (
          <div className="text-center py-8 px-4 bg-[#0f1428]/30 border border-dashed border-slate-800 rounded-2xl animate-fade-slide-up">
            <span className="text-2xl block mb-2">📚</span>
            <p className="text-xs text-slate-500 font-medium">No solved problems yet. Keep grinding!</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-5">
